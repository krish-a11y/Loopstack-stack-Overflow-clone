...
// page for adding question on the site
import RTE from "./RTE";
...
// component rapper that adds metor effet to the children and also add some css styling
...
type Question = Models.Document & QuestionDocument;
// function takes the question (in object form as props)
const QuestionForm = ({ question }: { question?: Question }) => {
  const { user } = useAuthStore();
  const [tag, setTag] = useState("");
  const router = useRouter();

  // tracking the form data that we get from the props
  ...
  // for tracking the rendering of the components
  ...
  const create = async () => {
    // if the attachment is not there throw  error
    if (!formData.attachment) throw new Error("Please upload an image");

    //    storing the attachment in the storage
    ...
    //    creating the question doc in the in the collection
    const response = await databases.createDocument(
      ...
      ID.unique(),
      {
        ...
        attachmentId: storageResponse.$id, //id  of the attachment that we stored
      },
    );

    //    loading the confetti animation as soon as the question  is created
    loadConfetti();

    return response;
  };

  //    updating the question function
  const update = async () => {
    // if the question that the user is updating not loaded from the appwrite
    if (!question) throw new Error("Please provide a question");

    //   checking if the  user has changed the attachement if he has then delete  the old one ans add new one
  ...
  const submit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    //   as the rendering still going
    setLoading(() => true);
    //   no error still now
    setError(() => "");

    try {
      // if the quesiton is already then run update if not then run create
      const response = question ? await update() : await create();

      // redirecting the  user to the page where the question is posted
      router.push(`/questions/${response.$id}/${slugify(formData.title)}`);
    } catch (error: any) {
      setError(() => error.message);
    }

    //   as the rendering over setting it to false
    setLoading(() => false);
  };
  return (
